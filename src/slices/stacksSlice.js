...
export const stacksSlice = createSlice({
    ...
    reducers: {
         ...
         */
        getUsers: (state) => {
            const users = state.stacks.map((stack) => stack.user);
            const filteredUsers = users.filter((item, index) => users.indexOf(item) === index);
        ...
        filterStacks: (state) => {
            ...
            if (state.filters.author) {
                state.searchResult = state.searchResult.filter((stack) => stack.user === state.filters.author);
            }
            // Filter by shared status
            if (state.filters.shared) {
                state.searchResult = state.searchResult.filter((stack) => stack.is_shared);
            }
            ...
            if (state.filters.created &&
                ...
                state.searchResult = state.searchResult.filter((stack) => {
                    const stackCreatedDate = startOfDay(new Date(stack.created)).getTime();
            ...
            if (state.searchValue) {
                state.searchResult = state.searchResult.filter((stack) => stack.id.toLowerCase().includes(state.searchValue.toLowerCase()) ||
...
export const { searchStack, getUsers, storeFilters, filterStacks } = stacksSlice.actions;
